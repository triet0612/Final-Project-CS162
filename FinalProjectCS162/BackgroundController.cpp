...
void BackgroundController::drawPicFromPath(const string& path, int x, int y, int color) {
	ifstream ifs(path);
	if (ifs.is_open()) {
		string tmp;
		int row = 0;
...
void BackgroundController::beginInterface() {
...
void BackgroundController::simpleBackground(int type) {
	int color;
	if (type == 1) {
...
void BackgroundController::loginInterface(int type) {
	int color;
	if (type == 1) {
		color = FG_BLUE | FG_GREEN | FG_I;
	}
