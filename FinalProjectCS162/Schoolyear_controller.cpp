...
bool Schoolyear_controller::CheckYear(string yearname) {
...
void Schoolyear_controller::saveYearList() {
	ofstream fout;
...
void Schoolyear_controller::loadYearList() {
	ifstream ifs;
...
void Schoolyear_controller::setupSchoolYearInputList(sll<InputRow>& inputList, sll<Button>& buttonList, sll<pair<int, int>>& pos) {
	for (int i = 0; i < 1; ++i) {
...
void Schoolyear_controller::setOnClickSubmitButton(Button& target, sll<InputRow>& inputList, string& res, bool& isOver) {
...
void Schoolyear_controller::setValidPosition(COORD c, int x, int y, int& cur, bool inTxt, bool inBtn, sll<pair<int, int>>& pos, ConsoleGraphics& graphics) {
	if (!inTxt && !inBtn) {
...
void Schoolyear_controller::renderInputProc(sll<InputRow>& inputList, sll<Button>& buttonList, sll<pair<int, int>>& pos, bool& isOver) {
	int cur = 0;
...
void Schoolyear_controller::createSchoolYear() {
	bool cancel;
...
int Schoolyear_controller::chooseOption(Table& table) {
	ConsoleGraphics* graphics = &ConsoleGraphics::getInstance();
	int type = -1;
	graphics->loopBoolean([&](pair<int, int> input) {
		int y = 0, x = 0;
...
void Schoolyear_controller::setupSchoolYearsTable(Table& table) {
	system("cls");
	table = Table(0, 0, 10);
...
int Schoolyear_controller::inputSchoolYearProc() {
	int type = 0;
	Table table;
...
void Schoolyear_controller::viewSchoolYear() {
	loadYearList();
...
Schoolyear Schoolyear_controller::getSchoolYear(string name) {
	loadYearList();
	sc.this_year = name;
...
void Schoolyear_controller::addSemesterToSchoolYear(string yearname) {
	sc.semester_control.addSemesterToSchoolYear(yearname);
}

void Schoolyear_controller::viewSemestersInYear() {
	//sc.semester_control.viewSemester();
}

void Schoolyear_controller::setupSchoolYearOptionsTable(Table& table, string yearname) {
	system("cls");
	table = Table(0, 0, 10);
...
int Schoolyear_controller::inputSchoolYearOptionProc(string yearname) {
	int type = 0;
	Table table;
...
void Schoolyear_controller::schoolYearOptions(string yearname) {
