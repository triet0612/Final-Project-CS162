...
bool Semester_controller::checkSemester(string name) {
...
void Semester_controller::saveSemester(string yearname) {
	ofstream fout;
...
void Semester_controller::create_semester(string yearname, sll<string> data) {
...
void Semester_controller::loadSemesterList(string yearname) {
	ifstream fin;
...
int Semester_controller::chooseOption(Table& table) {
...
void Semester_controller::setupSemestersTable(Table& table) {
	system("cls");
	table = Table(0, 0, 5);
...
int Semester_controller::inputSemesterTableProc() {
	int type = 0;
	Table table;
...
void Semester_controller::viewSemesters(string yearname) {
...
void Semester_controller::createCourseReg(semester& s) {
	s.courses.courseRegsController.modifyDatesReg();
	//s.course_reg.ModifyRegCourseDates();
	return;
}

void Semester_controller::setupSemesterInputList(sll<InputRow>& inputList, sll<Button>& buttonList, sll<pair<int, int>>& pos) {
...
void Semester_controller::setOnClickSubmitButton(Button& target, sll<InputRow>& inputList, sll<string>& res, bool& isOver) {
...
void Semester_controller::setValidPosition(COORD c, int x, int y, int& cur, bool inTxt, bool inBtn, sll<pair<int, int>>& pos, ConsoleGraphics& graphics) {
	if (!inTxt && !inBtn) {
...
void Semester_controller::renderInputSemesterProc(sll<InputRow>& inputList, sll<Button>& buttonList, sll<pair<int, int>>& pos, bool& isOver) {
	int cur = 0;
	ConsoleGraphics& graphics = ConsoleGraphics::getInstance();
...
void Semester_controller::renderCaution(string s) {
	TextBox notice = TextBox(5, 12, 40, 3, false, 0, 12).setText(s);
	notice.render();
}

void Semester_controller::renderAccept() {
	TextBox notice = TextBox(5, 12, 40, 3, false, 0, 10).setText("input successfully, loading ...");
	notice.render();
}

sll<string> Semester_controller::inputSemesterProc(bool& isCancel) {
...
void Semester_controller::addSemesterToSchoolYear(string yearname) {
	bool isCancel = false;
	system("cls");
...
void Semester_controller::setupSemesterInfoTable(Table& table, int id, string yearname) {
	system("cls");
	table = Table(0, 0, 3);
...
void Semester_controller::semesterInfoTableProc(int id, string yearname) {
	Table table;
	setupSemesterInfoTable(table, id, yearname);
}

void Semester_controller::viewSemesterInfoTable(int id, string yearname) {
	system("cls");
...
void Semester_controller::setupSemesterOptionsTable(Table& table) {
	table = Table(0, 7, 5);
...
int Semester_controller::inputSemesterOptionTableProc() {
	int type = 0;
	Table table;
...
void Semester_controller::viewScoreClass(string semestername, string yearname) {
...
void Semester_controller::setCurrentSemester(string yearname, string semester) {
	ofstream ofs("Data/CurrentSemester.txt");
	ofs << yearname << "\n";
	ofs << semester << "\n";
	ofs.close();
...
void Semester_controller::viewSemesterOptions(int id, string yearname) {
